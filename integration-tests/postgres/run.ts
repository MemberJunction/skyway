/**
 * PostgresProvider integration test — exercises the full Skyway lifecycle
 * against a real PostgreSQL instance.
 *
 * Phases:
 *   1. Migrate (apply 2 fresh migrations)
 ...
 * point invoked by the integration CI workflow. We don't use vitest because
 * vitest's parallelism plays poorly with single-DB integration setup, and
 * because keeping it as a plain Node script makes the CI failure output
 * trivial to read.
 */

import { Skyway } from '../../packages/core/src/index';
import { PostgresProvider } from '../../packages/postgres/src/index';

const DB_CONFIG = {
  Dialect: 'postgresql' as const,
  Server: process.env.SKYWAY_PG_HOST ?? 'localhost',
  Port: Number(process.env.SKYWAY_PG_PORT ?? 5432),
  Database: process.env.SKYWAY_PG_DATABASE ?? 'skyway_integration',
  User: process.env.SKYWAY_PG_USER ?? 'postgres',
  Password: process.env.SKYWAY_PG_PASSWORD ?? 'postgres',
};
...
async function main() {
  console.log('=== Skyway PostgreSQL Integration Test ===');
  ...
  const skyway = new Skyway({
    Provider: provider,
    Migrations: {
      Locations: ['./integration-tests/postgres/migrations'],
  ...
  try {
    ...
    // Phase 2 — Info reports both as SUCCESS
    console.log('\n--- 2. Info ---');
    ...
    // Phase 4 — Re-run Migrate is a no-op
